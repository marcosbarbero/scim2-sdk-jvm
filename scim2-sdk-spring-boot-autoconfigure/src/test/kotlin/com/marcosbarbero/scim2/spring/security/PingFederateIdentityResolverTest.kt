...
class PingFederateIdentityResolverTest {

    private val resolver = PingFederateIdentityResolver()
    ...
    @Test
    fun `extracts memberOf claim as roles`() {
        val jwt = mockJwt(mapOf("sub" to "ping-user", "memberOf" to listOf("cn=Admins", "cn=Users")))
        setAuthentication(jwt)

        val context = resolver.resolve(request)

        context.roles.shouldContainExactlyInAnyOrder("cn=Admins", "cn=Users")
    }

    @Test
    fun `extracts groups claim as roles`() {
        val jwt = mockJwt(mapOf("sub" to "ping-user", "groups" to listOf("GroupA", "GroupB")))
        setAuthentication(jwt)

        val context = resolver.resolve(request)

        context.roles.shouldContainExactlyInAnyOrder("GroupA", "GroupB")
    }

    @Test
    fun `combines memberOf and groups claims`() {
        val jwt = mockJwt(
            mapOf(
                "sub" to "ping-user",
                "memberOf" to listOf("cn=Admins"),
                "groups" to listOf("GroupA"),
            ),
        )
        setAuthentication(jwt)
        ...
        context.roles.shouldContainExactlyInAnyOrder("cn=Admins", "GroupA")
    }

    @Test
    fun `returns empty roles when no memberOf or groups claims`() {
        val jwt = mockJwt(mapOf("sub" to "ping-user"))
        setAuthentication(jwt)
    ...
    fun `extracts principal from sub`() {
        val jwt = mockJwt(mapOf("sub" to "ping-user-123"))
        setAuthentication(jwt)

        val context = resolver.resolve(request)

        context.principalId shouldBe "ping-user-123"
    }
}
